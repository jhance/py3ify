...
class RemoveUnicodeCompatible(VisitorBasedCodemodCommand):
    ...
    """
    def leave_ClassDef(self, original: libcst.ClassDef, updated: libcst.ClassDef) -> libcst.ClassDef:
        resulting_decorators = []
        for decorator_wrapper in updated.decorators:
            decorator = decorator_wrapper.decorator
            if not m.matches(decorator, m.Attribute(
                value=m.Name("six"),
                attr=m.Name("python_2_unicode_compatible"),
            )):
